SELECT 
  Min(D.Key) AS Key,
  CASE
    WHEN (Count(D.LO_CD4) = 0 AND
      COUNT(D.LO_CD8) = 0 AND
      COUNT(D.LO_SEB) = 0 AND
      AVG(D.negctrl_CD4_Resp) + AVG(D.negctrl_CD8_Resp) <= 0.2) THEN 'PASS'
    ELSE
    (
      ...
      CASE WHEN (AVG(D.negctrl_CD4_Resp) + AVG(D.negctrl_CD8_Resp) > 0.2) THEN 'HI_BKG' ELSE '' END
    )
  END AS Verdict,
  D.Run,
  D."EXPERIMENT NAME",
