...
A.PTIDTYPE,
CAST(NULL AS VARCHAR) AS SPECTYPE,
CAST(NULL AS VARCHAR) AS ISOLATION,
CAST(NULL AS VARCHAR) AS ADDITIVE,
CAST(NULL AS VARCHAR) AS CRYOSTAT,
...
-- AnalysisPlan007 can't select 'A._fcsfile.Sample.Property.NUMVIALS' when NUMVIALS is already present
--A.NUMVIALS,
A.RUNGROUPID,
--A.VIAL1_ID,
--A.VIAL2_ID,
...
A.REPLACE,
A.MODDT,
 CASE WHEN (A.ANALYSIS_COMMENT IS NULL) THEN('') ELSE ('(Analysis:)' || A.ANALYSIS_COMMENT) END ||
