...
A.WELL_ID,
CASE
WHEN IFDEFINED(FCSAnalyses.FCSFile.Keyword.WELLROLE) IS NOT NULL THEN IFDEFINED(FCSAnalyses.FCSFile.Keyword.WELLROLE)
...
A.RUNNUM,
A.RELIABLE,
 CASE WHEN (A.ANALYSIS_COMMENT IS NULL) THEN('') ELSE ('(Analysis:)' || A.ANALYSIS_COMMENT || chr(10)) END ||
 CASE WHEN (A.FCS_COMMENT IS NULL) THEN('') ELSE ('(File:)' || A.FCS_COMMENT || chr(10)) END ||
 CASE WHEN (A.RUN_COMMENT IS NULL) THEN('') ELSE ('(Run:)' || A.RUN_COMMENT || chr(10)) END ||
 CASE WHEN (A.SAMPLE_COMMENT IS NULL) THEN('') ELSE ('(Sample:)' || A.SAMPLE_COMMENT || chr(10)) END ||
 CASE WHEN (A.SAMPLE_COMMENTS IS NULL) THEN('') ELSE ('(Sample:)' || A.SAMPLE_COMMENTS || chr(10)) END
AS COMMENTS,
-- pass through
A._fcsfile,
A._well,
